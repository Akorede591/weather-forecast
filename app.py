...
def generate_pipeline_flowchart():
    ...
    # Increased size constraints and adjusted graph scaling properties
...
def generate_algo_flowchart(selected_feats):
    ...
    # Dynamic terminal display node based on selected output space
...
target_column = st.sidebar.selectbox("🎯 Select Target Column to Forecast", options=selectable_targets, index=selectable_targets.index(default_target))
...
if st.button("🚀 Execute Hybrid Pipeline Optimization"):
    ...
    features = [col for col in clean_df.columns if col not in excluded and col != target_column]
