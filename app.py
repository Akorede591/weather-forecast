...
def generate_pipeline_flowchart():
    ...
    dot.node('A', 'Weather Data\n(Local or Uploaded CSV)')
...
# --- ADDED SECTION: OPTIONAL CSV FILE UPLOADER ---
st.markdown("### 📂 Custom Dataset Upload (Optional)")
uploaded_file = st.file_uploader("To test with your own data, drag and drop a weather CSV file here. If left blank, the system defaults to the built-in dataset.", type=["csv"])
st.write("---")
...
# If a file is uploaded, use it. Otherwise, use your default background dataset.
if uploaded_file is not None:
    try:
        df_raw = load_and_prepare_data(uploaded_file)
        st.success("📊 Using uploaded custom dataset!")
    except Exception as e:
        st.error(f"Error parsing uploaded file. Recheck your CSV format.")
        st.stop()
else:
    try:
        df_raw = load_and_prepare_data(DEFAULT_CSV_PATH)
        st.info("ℹ️ Running system using default background dataset (`weather_prediction_dataset2.csv`).")
    except Exception as e:
        st.error(f"⚠️ Default dataset missing inside: `{DEFAULT_CSV_PATH}`. Please upload a CSV above to proceed.")
        st.stop()

# Show dataset preview structure
with st.expander("🔍 Preview Current Active Data Columns"):
...
if st.button("🚀 Execute Hybrid Pipeline Optimization"):
    ...
    # Remove text/unsupported meta headers
    ...
    if X_raw.shape[1] == 0:
        st.error("The selected data does not contain valid numeric attributes to process.")
        st.stop()
    ...
    tab1, tab2, tab4, tab4_matrix = st.tabs([
